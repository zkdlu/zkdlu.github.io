...
호출 결과가 JSON 형태로 데이터가 오게 되는데, 스프링부트의 MessageConverter가 Jackson 라이브러리를 이용해 Reflection으로 객체를 생성하게 되는데  기본 생성자가 없을 경우 Jackson 라이브러리가 deserialize 할 수 없어 발생하는 예외이다. 



**결론**

Feign Client는 NoArgsConstructor를 두자! 
